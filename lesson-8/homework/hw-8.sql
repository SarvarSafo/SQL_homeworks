create database sql_homework_8
go
use sql_homework_8
...
select max(quantity)as maximum, min(quantity) as minimum 
from Orders;
...
select productid, sum(saleamount) as amount from Sales
group by productid
...
SELECT p.ProductName, COUNT(s.ProductID) AS NumberOfSales
FROM Products p
