create database sql_homework_9
go
use sql_homework_9
...
select payments.orderid, orders.orderid
		from payments
		inner join
...
select Orders.ProductID, Orders.Quantity, Products.StockQuantity
		from Orders
		inner join 
		Products on Orders.ProductID = Products.ProductID
...
select concat_ws(' ', FirstName, LastName) as name, Sales.ProductID
		from Customers
		inner join
